...
class Quiz extends React.PureComponent<ChildProps<IQuizProps, IResponse>, IQuizState> {
	...
	componentDidUpdate(prevProps: ChildProps<{}, IResponse>) {
		...
		if (Page && prevProps.data!.Page !== Page) {
			if (Page.pageInfo.hasNextPage) {
				...
				const availableIds = allIds.filter(id => !correctIds.includes(id) && !wrongIds.includes(id) && !skipIds.includes(id))
				const selectedId = this.state.selectedId === -1 ? getRandomValue(availableIds) : this.state.selectedId
	...
	handleCorrect = () => {
		...
		const availableIds = allIds.filter(id => !newCorrectIds.includes(id) && !wrongIds.includes(id) && !skipIds.includes(id))
	...
	handleWrong = () => {
		...
		const availableIds = allIds.filter(id => !correctIds.includes(id) && !newWrongIds.includes(id) && !skipIds.includes(id))
		const selectedId = getRandomValue(availableIds)
	...
	handleSkip = () => {
		...
		const availableIds = allIds.filter(id => !correctIds.includes(id) && !wrongIds.includes(id) && !newSkipIds.includes(id))
		const selectedId = getRandomValue(availableIds)
		...
		return (
			<Container>
				...
				<div style={{ marginTop: 3.5 }}>
					<Button positive={true} onClick={this.handleCorrect}>Correct - {correctIds.length}</Button>
					<Button negative={true} onClick={this.handleWrong}>Wrong - {wrongIds.length}</Button>
					<p>Total - {availableIds.length}</p>
				</div>
				<Description animeId={selectedId} type={this.props.type} handleSkip={this.handleSkip} />
			</Container>
		)
	}
}
...
const GET_ALL_ANIME = gql`
	query GetAllAnime($page: Int, $type: MediaType, $popularity_greater: Int, $category: String, $format_in: [MediaFormat]) {
		Page(perPage: 50, page: $page) {
			pageInfo {
				currentPage
				hasNextPage
			}
			media(type: $type, popularity_greater: $popularity_greater, tagCategory_in: [$category], format_in: $format_in) {
				id
			}
		}
	}

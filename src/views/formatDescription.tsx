...
const closeEnough = (s1: string, s2: string): boolean =>
  get(s1, s2) / s1.length <= 0.3

const replaceAll = (s1: string, s2: string): string =>
  s1.replace(new RegExp(`\\b${s2}\\b`, "g"), "_".repeat(s2.length))
...
const getImportant = (s: string): string[] => {
  ...
  return words.filter(
    w => w && isUpper(w[0]) && !["The", "Or", "A"].includes(w)
  )
}
